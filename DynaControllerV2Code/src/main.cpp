/* Piezo drums are disabled but could be implemented if wanted. 
   Uses a Piezo sensor (percussion / drum) to send USB MIDI note on
   messages, where the "velocity" represents how hard the Piezo was
   tapped.

   How Piezos are wired: 
   Connect a Piezo sensor to analog pin.  This example was tested
...
This circuit uses a library for the MPR121 12-channel Capacitive touch sensor
...
//This revision is modified for the Dynacontroller V1 PCB, which uses Teensy 4.0 instead of 3.2. Some pinouts are different. 
//Piezo Drum stuff not used, but still in the code (the main controller has piezos installed and hardware for tuning, but it's not implemented. 
//all control is through MPR121 touchpoints)
// Make sure to cut the ADDR traces on the back of the second MPR121 breakout board, and pull the addr pin to high/3.3V to set it to 0x5B
//helpful links: teensy wire library: https://www.pjrc.com/teensy/td_libs_Wire.html
...
#define USB_MIDI_SERIAL_KEYBOARD
...
#define USE_ZONE_EFFECT 0

#define KEYBOARD_SELECT_PIN 22
#define MIDI_SELECT_PIN 23

//This is the duration between sends of midi signal in Milliseconds.
#define TRIGGER_DURATION 30

// This defines what modes we have to choose from 
// USE_CC_TIMER will have a repeating fire CC Signal and is deprecated
// USE_NOTE_ON_OFF will fire a MIDI note press and release
// USE_KEYBOARD will fire keyboard press and release mode.
// The mode will use the switch on the PCB at setup to select keyboard or midi output.  
// To use one or the other based on the switch, set all to 0. Setting any to 1 will force that mode regardless of switch position. 
// This will work for the original controller which does not have a switch connected so pins will always be pulled up HIGH.

#define USE_CC_TIMER 0
#define USE_NOTE_ON_OFF 0
#define USE_KEYBOARD 0

#define KEYBOARD_SELECTED 1
#define MIDI_SELECTED 2
// defaults to MIDI Output.
uint8_t mode = MIDI_SELECTED;

//currently using MIDI out, but might use keystrokes in the future. A toggle pin with common ground between is added to 22 to represent MIDI, and 23 to represent Keys. if implemented, use jumper to indicate what output is preferred (midi or keys)

// LED Stuff
#define DATA_PIN    8 //used to be pin 3 on teensy3.2
// clock pin wired to pin 7, but not implemented in code
...
#define MPR121_TOUCH_THRESHOLD_DEFAULT 10  ///< default touch threshold value (was 12)
#define MPR121_RELEASE_THRESHOLD_DEFAULT 3 ///< default relese threshold value (was 6)
...
//const uint8_t notesA[numElectrodes] = {36, 38, 40, 43, 45, 47, 48, 50, 52, 55, 57, 60}; //v1 Notes Not Used Anymore
//const uint8_t notesA[numElectrodes] = {58, 50, 48, 60, 49, 57, 46, 52, 36, 38, 55, 51}; //PCB Edition Dynacontroller Layout
  const uint8_t notesA[numElectrodes] = {24, 36, 48, 45, 33, 57, 60, 55, 53, 41, 43, 31}; //Large Dynacontroller Current Map
  const uint16_t keysA[numElectrodes] = {KEY_G, KEY_H, KEY_F, KEY_D, KEY_V, KEY_C, KEY_B, KEY_N, KEY_J, KEY_K, KEY_M, KEY_COMMA};

//const uint8_t notesB[numElectrodes] = {37, 39, 41, 42, 46, 49, 51, 53, 54, 56, 58, 59}; //v1 Notes Not Used
//const uint8_t notesB[numElectrodes] = {1, 1, 1, 1, 43, 53, 40, 45, 54, 47, 56, 1}; //PCB Edition Dynacontroller Layout
  const uint8_t notesB[numElectrodes] = {1, 2, 3, 4, 40, 29, 52, 50, 28, 38, 26, 5}; //Large Dynacontroller Current Map
  const uint16_t keysB[numElectrodes] = {KEY_Z, KEY_BACKSLASH, KEY_SPACE, KEY_0, KEY_A, KEY_L, KEY_P, KEY_W, KEY_S, KEY_X, KEY_PERIOD};
...
void setup() {
  ...
  Wire.begin(); //added by drc
  //Wire.begin(0x5B); //added by drc
  Serial.begin(9600); //uncomment to enable serial monitoring
  pinMode(LED_BUILTIN, OUTPUT); //added by drc

  pinMode(KEYBOARD_SELECT_PIN, INPUT_PULLUP);
  pinMode(MIDI_SELECT_PIN, INPUT_PULLUP);
  Wire.setSDA(18); //use a 4.7k pullup resistor. same for 3.2 or 4.0
  Wire.setSCL(19); //use a 4.7k pullup resistor. same for 3.2 or 4.0
 ...
 if (!capB.begin(0x5B)) {
  ...
  if ( digitalRead(KEYBOARD_SELECT_PIN) == LOW){
    mode = KEYBOARD_SELECTED;
    Serial.println("Keyboard Output Selected");
  }
  else if (digitalRead(MIDI_SELECT_PIN) == LOW) {
    mode = MIDI_SELECTED;
    Serial.println("MIDI Output Selected");
  };
...
void checkElectrodes(){
  ...
  for (uint8_t i=0; i<numElectrodes; i++) { //changed i<0 to i<numElectrodes
    ...
    if ((currtouchedA & _BV(i)) && !(lasttouchedA & _BV(i)) ) {
      ...
      if (USE_NOTE_ON_OFF or mode == MIDI_SELECTED){
        usbMIDI.sendNoteOn(notesA[i], 127, channel); // note, velocity, channel
      }
      
      if (USE_KEYBOARD or mode == KEYBOARD_SELECTED){
         Keyboard.press(keysA[i]);
      }
